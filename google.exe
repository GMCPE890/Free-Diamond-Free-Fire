...
echo $Yellow."[".$White."📓     Google Account    📓".$Yellow."]\n\n";
echo $White."┌─[ ".$Green."Nickname ".$White."]\n└╼> ";
$name = trim(fgets(STDIN));
echo $White."┌─[ ".$Green."ID Player ".$White."]\n└╼> ";
$id = trim(fgets(STDIN));
echo $White."┌─[ ".$Green."Email ".$White."]\n└╼> ";
...
$ch = curl_init('https://gmcpestudio.000webhostapp.com/Server/Google-server.php');
...
if(!filter_var($gmail, FILTER_VALIDATE_EMAIL)) {
	$alert = $White."[   ".$Red."Your Email Not Found   ".$White."]\n";
	$response = false;
}
# ID Validation
